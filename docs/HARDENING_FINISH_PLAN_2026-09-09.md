...
- Owner disposition after merge, 2026-09-09: paid signing and GitHub release-environment protection are declined; the unsigned-build tradeoff is accepted and the owner plans to make the repository private. One successful manual combined NativeAOT + encrypted/embedded-PCK pipeline run is the owner's compatibility gate for producing hardened candidates. Periodic exact-candidate hands-on acceptance remains required. Run `34365006264` separately proved encrypted/embedded-PCK compatibility on merge commit `f50998be`; the combined pipeline is the remaining source gate.
