...
| Contains a tutorial | **YES** | `src/Onboarding/FirstSessionGuidanceController.cs` — a persisted, skippable, spotlighted step sequence with a dedicated tutorial character presenter, replayable from Settings ("Show Tutorial Again"). Every distribution ships a walkthrough; itch.io teaches the shorter `TutorialStepIds.ItchIo` sequence, which drops the Paint Room, Buddy Studio and Work Mode chapters it does not ship. Covered by `DemoCleanSaveAcceptanceTests`. |
