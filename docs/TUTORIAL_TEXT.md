...
| 7 | `create_buddy` | First, a buddy of your own to work on. Press New to mint a fresh character — the one you paint from here on. |
| 8 | `name_buddy` | Give him a name. Type it in the name box and press Enter. |
| 9 | `select_paint_brush` | Pick up the Brush — it is the one you will use most. |
| 10 | `select_paint_color` | Now pick any colour that takes your fancy. |
| 11 | `paint_buddy` | Go on, paint something across Buddy's torso. |
| 12 | `save_paint_buddy` | Happy with it? Save the character to keep your work. |
| 13 | `use_painted_buddy` | Saving keeps it; Use Character puts it on the real Buddy. Choose Use Character to apply it and head back. |
| 14 | `admire_painted_buddy` | Look at that. Genuine artistry — Buddy has never looked better. Wear it with pride. |
| 15 | `open_paint_background` | Buddy has a new look — the room deserves one too. Open Paint ▸ Background. |
| 16 | `select_background_spray` | Grab the Spray can this time. |
| 17 | `select_background_color` | Pick any colour you like from the palette. |
| 18 | `paint_background` | Now spray somewhere on the room behind Buddy. |
| 19 | `float_paint_background_panel` | The tool panel is in the way. Drag it by its title bar right out of the game window and park it off to the side — the 📌 button does the same. It becomes its own desktop window, and you get the whole background to admire. |
| 20 | `save_exit_paint_background` | Lovely. Choose Save and Exit to keep it. |
| 21 | `open_buddy_studio` | One more workshop to show you. Open Buddy Studio — this is where Buddy gets his hair, glasses, hats and everything else. |
| 22 | `select_nose_category` | Start with the Nose category. |
| 23 | `select_nose_button_style` | Click the Button nose once. A single click only previews it — watch it appear on Buddy in the preview window. |
| 24 | `buy_studio_item` | Like it? Buy it with the button on the right. Double-clicking the style itself does the same thing. |
| 25 | `equip_studio_item` | Now equip it. You can come back here any time to equip something else, or switch a slot back to its free default to take it off again. |
| 26 | `save_buddy_studio` | Save it, or Buddy loses his brand-new nose. |
| 27 | `exit_buddy_studio` | That is Buddy Studio. Exit when you are ready. |
| 28 | `admire_studio_buddy` | Now that is a nose. Painted, kitted out, and frankly better dressed than most of us — Buddy is looking sharp. |
| 29 | `enter_work_mode` | Last stop. Work Mode shrinks Buddy into a tiny companion that sits on top of your real desktop and keeps earning while you get on with things. Open Work. |
| 30 | `drag_work_companion` | Hold the left mouse button on Buddy, on the computer, or on the blue bar, then move the mouse to drag the companion wherever you want it. |
| 31 | `resize_work_companion` | Hold the left mouse button on the ↘ resize button and move the mouse to make the companion bigger or smaller. |
| 32 | `toggle_work_counter` | That little screen counts what you have done. Click it to switch between this session and your lifetime total. |
| 33 | `exit_work_mode` | Done working? Double-click Buddy, or press X, to come back. |
| 34 | `farewell` | And that is everything. If you ever forget what something does, hit the ? button in the title bar and hover over it — Help mode will explain anything on screen. Go have fun with him. Goodbye! |

All 34 ids are prefixed `demo.onboarding.` in `TutorialStepIds`.
