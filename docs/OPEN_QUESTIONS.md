...
Per the planning rule in `AGENTS.md` and NFR-006.5, implementation agents must not invent answers to product-behavior questions. When a new gap is found, add an entry here, stop work that depends on it, and ask the owner. When the owner decides, move the decision into `docs/DECISIONS.md` and remove the entry.
