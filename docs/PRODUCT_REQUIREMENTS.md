...
3. **FR-008.3:** WHEN the buddy completes a catch of a safely thrown catchable object THEN it SHALL grant `+1` mood once for that throw/catch event. The Soccer Ball is foot-only under FR-009.12 and is not catchable.
...
12. **FR-009.12:** WHILE the buddy considers a Soccer Ball THEN it SHALL NOT enter the ordinary catch/scoop/hold lifecycle or gain a clean-catch reward. The only hand-attachment exception SHALL be FR-009.13's explicit good-mood corner rescue. WHEN the player has touched the ball and the ball subsequently rolls toward the buddy without touching a side wall or ceiling THEN the buddy MAY trap it underfoot; floor contact SHALL NOT revoke that trap permission. WHEN wall or ceiling contact revokes trapping THEN the buddy MAY still kick a low reachable ball, and another player touch SHALL restore trap permission.
13. **FR-009.13:** WHILE a conscious buddy is Content or Delighted AND a free Soccer Ball is sensed AND no higher-priority behavior owns actuation THEN the buddy SHALL chase and kick the ball. WHEN choosing an autonomous kick away from a wall THEN it SHALL choose deterministically from straight-forward or a non-zero authored arc. WHEN the ball is stranded within the authored side-wall distance THEN the buddy SHALL pick it up, move and face inward away from the wall while continuously watching it, place it on the floor in front, and kick it inward. WHILE the player continuously holds the Soccer Ball THEN a Content or Delighted buddy SHALL continuously keep its head and rendered eyes on the ball while travelling and repeatedly increase horizontal separation, pausing briefly after every five seconds of walking, without attaching to or colliding exceptionally with it. WHEN the player releases the ball THEN receive retreat SHALL end immediately and free-ball chase/play SHALL resume. WHEN ambient autonomy tests whether to jump an obstacle THEN a Soccer Ball SHALL NOT produce an obstacle-hop request, while other loose objects SHALL remain eligible.
...
2. **FR-010.2:** WHEN the Shotgun fires THEN it SHALL create `6` physical continuous-collision-detection pellets, choose a new seeded-random spread half-angle from `12–20` degrees for that shot, eject one cosmetic red shell, consume one of its `5` loaded shells, and enforce at least `0.9` seconds before the next shot. The next primary press SHALL cycle the pump rather than fire; only a later press may fire the chambered shell. Pellet knockback SHALL fall with distance, from a point-blank six-pellet maximum equal to twice the Grenade's center shove to the pre-existing physical-contact knockback as its non-decreasing floor. WHEN a Shotgun reload is initiated THEN it SHALL take `2` seconds; reserve ammunition SHALL be unlimited.
...
6. **FR-017.6:** WHEN no saved presentation settings exists THEN V-sync SHALL be On, anti-aliasing SHALL be `2x` MSAA, Master and SFX volume SHALL each be `50%`, Mute in Work Mode SHALL be On, Screen Shake SHALL be On, Reduced Motion and Reduced Particles SHALL be Off, and Photosensitivity-Safe Effects SHALL be On.
