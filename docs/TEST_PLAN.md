...
- The `grab_dangle` scenario lifts the buddy independently by head, torso, both hands, and both feet. Every unsupported case must disable all active drive and resistance while retaining passive structural topology within `24 px`; the same bound applies while unconscious, grounded grabs retain standing drive, every body remains finite, and release resumes drive (`--fixed-fps 120`).
