...
2. `docs/MODDABILITY_AND_WORKSHOP_SECURITY_SOURCE_ALIGNMENT_2026-09-07.md` — research-backed architecture supplement for designing the new systemic systems around high declarative moddability while keeping Workshop content non-executable and hostile-data validated. It controls new definition/capability/event/action/package seams and the trust boundary for future code mods.
3. `docs/NEXT_FEST_THREE_BUILD_SCOPE_SOURCE_ALIGNMENT_2026-09-07.md` — owner-authorized correction defining the three cumulative Steam build surfaces: Normal Demo, Next Fest Demo Update, and Full Release. It supersedes any wording that removes existing/pre-approved Normal Demo content for Next Fest curation or treats the Normal Demo and Next Fest Demo as the same build.
4. `docs/NEXT_FEST_DEMO_SYSTEMIC_SANDBOX_VERTICAL_SLICE_2026-09-07.md` — detailed Next Fest vertical-slice plan. Read through item 3: curation applies only to newly introduced systemic systems/content, never by removing existing Normal Demo functionality.
5. `docs/FULL_RELEASE_MULTI_BUDDY_SCENES_SOURCE_ALIGNMENT_2026-09-07.md` — owner-authorized Scene/multi-Buddy architecture supplement. It defines the account/Buddy state split, Scene documents and production multi-Buddy runtime. Where its distribution wording conflicts with items 3–4, those newer Demo scope decisions control. Buddy-to-Buddy social AI/interactions remain deferred.
6. `docs/DECISIONS.md` — owner-confirmed decisions. Historical Phase B/Phase C deferrals remain historical where superseded by newer owner-authorized supplements.
7. `docs/M5_5_PHASE_B_PAINTING_SOURCE_ALIGNMENT.md` — normative Milestone 5.6 painting supplement, locked painting behavior, architecture, budgets, task order, and verification.
8. `docs/M5_5_CHARACTER_EDITOR_SOURCE_ALIGNMENT.md` — normative Phase A supplement and historical A0 scope gate. Its statements that painting is deferred are superseded by item 7; its trusted visual/character architecture remains binding.
9. `docs/PRODUCT_REQUIREMENTS.md` — baseline observable behavior and acceptance criteria, as supplemented by items 1–5, 7, and 8.
10. `docs/RAGDOLL_AND_GAMEPLAY_SPEC.md` — physics/gameplay contract.
11. `docs/ARCHITECTURE.md` — baseline ownership, interfaces, data flow, and failure behavior, as supplemented by items 1–5, 7, and 8.
12. `docs/TEST_PLAN.md` and `docs/ROADMAP.md` — baseline verification and milestone order, as supplemented by items 1–5 and 7.
13. `docs/AGENT_VERIFICATION_AND_E2E.md` — baseline interactive verification workflow and end-to-end journey suite.
14. `docs/CHARACTER_EDITOR_WORKSHOP_PLAN.md` — Phase A historical task contracts and original Workshop planning. Its Steamworks.NET/Phase C deferral wording is superseded by item 1; trusted character/package validation concepts remain historical design evidence where they do not conflict.
15. `docs/GODOTSTEAM_WORKSHOP_AND_SOCIAL_FEATURES_IMPLEMENTATION_PLAN_2026-08-25.md` — detailed Workshop research and task contracts, as activated and corrected by items 1–4.
16. `docs/REFERENCE_RESEARCH.md` — clean-room reference evidence and technical sources.
17. `docs/OPEN_QUESTIONS.md` — decisions awaiting owner confirmation; do not implement behavior an open question affects until it is resolved by a higher-priority owner decision.
...
- **High moddability is an architecture requirement for the new systemic work as of 2026-09-07.** New construction, properties, materials, devices, weapons/projectiles, damage/repair, Scene and Blueprint systems should expose stable engine-free definitions, registries, capabilities and semantic event/action surfaces wherever practical. Workshop remains a non-executable hostile-data path; advanced scripted mods require a separate future trust boundary.
...
- Store trusted first-party tunable/static authoring content in typed Godot Resources, but compile new moddable systemic definitions into engine-free semantic definitions/registries. Hostile Workshop packages use their own bounded JSON/media compiler into those same semantic definitions and never reuse the trusted Godot Resource loader.
- New systemic behavior should prefer reusable project-owned capabilities and stable semantic events/actions over bespoke `switch (DefinitionId)` branches. A capability is `CoreOnly` by default until its hostile-input parameters, performance cost and Workshop exposure are explicitly reviewed.
- Mod/package identity is semantic and provider-qualified; do not use Resource paths, node paths, class names, filenames, Steam display names or Workshop item IDs as gameplay definition identity. Workshop IDs are provenance only.
- Declarative behavior graphs may invoke only registered typed events, conditions and actions. They receive semantic context, never Godot Nodes/RIDs or CLR objects. Immediate recursion is prohibited; cycles cross explicit delayed/tick boundaries and are subject to event/action/spawn budgets.
- Workshop behavior cannot directly mutate wallet, unlocks, achievements, Steam subscriptions/publishing, arbitrary save files or other account-global privileged state. It may only use specifically exposed gameplay capabilities.
- Store progress, Buddy identity documents, Scene documents/assets, character documents, declared local paint PNGs, validated content packs, imported Workshop copies, and Workshop provenance only in their separately owned/versioned persistence boundaries.
...
- Character customization, painting, and Workshop-imported character/room content are visual-only by construction under their existing package types. They must never reach rig geometry, collision, mass, drives, forces, connectors, damage, mood, economy, or tool rules.
...
- PNG decode/encode, JSON, file I/O, Workshop staging, hashing, content-pack compilation and dependency resolution never occur on the fixed physics tick. Godot texture creation/update and GodotSteam calls remain on the main thread where required.
...
- Workshop content is hostile data. V1 and later specifically authorized package types accept only explicitly versioned declarative formats and exact path/ID/property/capability whitelists. Never `GD.Load`/`ResourceLoader` Workshop content and never accept Workshop scenes, Resources, PCKs, scripts, shaders, DLLs, native libraries, executables, arbitrary meshes, or generalized in-process code mods.
- A .NET `AssemblyLoadContext` is never treated as a security sandbox. If advanced scripted mods are later authorized, they require a separate explicit trust model such as local full-trust code, OS process isolation, or a separately threat-modeled capability-restricted WebAssembly host.
- Workshop package updates are revalidated from an immutable snapshot based on their new content hash. Publisher identity, Steam tags or prior validation do not grant the changed bytes trust. Preserve a last-known-good local validated copy when feasible so a bad update cannot destroy an existing Scene.
...
- Follow the numbered task/source-alignment document for the feature being changed. Moddability/security architecture follows `docs/MODDABILITY_AND_WORKSHOP_SECURITY_SOURCE_ALIGNMENT_2026-09-07.md`; three-build scope follows `docs/NEXT_FEST_THREE_BUILD_SCOPE_SOURCE_ALIGNMENT_2026-09-07.md`; Next Fest systemic/Scene work follows those supplements plus `docs/NEXT_FEST_DEMO_SYSTEMIC_SANDBOX_VERTICAL_SLICE_2026-09-07.md` and the multi-Buddy architecture supplement; painting work follows `docs/M5_5_PHASE_B_PAINTING_SOURCE_ALIGNMENT.md`; existing Workshop v1 work follows `docs/M6_WORKSHOP_SOURCE_ALIGNMENT_2026-08-25.md` and the GodotSteam implementation plan.
- While implementing any new systemic definition, property, material, device or action, explicitly decide whether the capability is `CoreOnly`, `LocalDeclarative` or `WorkshopDeclarative`; do not accidentally expose trusted internals just because the first-party object needs them.
- Prefer first-party content that exercises the same registry/capability/event/action code paths future declarative mods will use. Keep raw authoring formats separate: internal `.tres` may be trusted source data, while UGC remains validated JSON/approved media compiled into engine-free domain definitions.
...
- Contraption Workshop sharing is authorized only after the local Blueprint schema/validator is stable and only as the declarative Demo-compatible/Full-required package model in the Next Fest supplement. Broader declarative content packs follow the modding supplement and require their own implementation gate; arbitrary executable mods remain unauthorized.
...
- Workshop-exposed capabilities additionally require hostile-input and resource-budget tests, including unknown IDs, forbidden capability exposure, invalid numeric values, dependency cycles, recursive signal graphs, spawn storms, oversized assets/data, path traversal/links, content-hash changes and missing-mod Scene recovery.
...
- Never silently change confirmed resolution, controls, budgets, path whitelist, AppID ownership, package schema, capability exposure or scope. Record owner-approved changes before implementation.
- Keep debug visualizers and tuning panels behind development-build guards.
- Treat warnings, invalid UVs, seam jumps, non-finite coordinates, unexpected paint/package paths, dimension/byte-cap violations, hash mismatches, save migration failures, failed image writes, dependency failures, behavior-budget overruns, main-thread violations, Steam callback mismatches, lost Scene switching/editor recovery, and lost window recovery as actionable failures.
...
- Workshop package import/export is authorized only for the specifically approved declarative content types. It must not become a route for arbitrary Resources, scripts, executable content or generalized full-trust mods.
...
- Never add slow steps to `CI / quick`. It is the per-commit feedback loop and must stay under roughly a minute. New scenarios, journeys, capture gates, native addon gates, Workshop fuzz/hostile-package gates and soaks belong in pull-request workflows.
...
- Automated tests cover normal, boundary, failure, memory, threading, hostile-input and resource-budget paths appropriate to the feature.
...
- New moddable systemic content exposes stable semantic definitions/capabilities rather than requiring package data to name CLR/Godot implementation types.
- No selectable UI element advertises an unimplemented or deferred feature.
- Paint and imported UGC remain inside their authorized semantic/package boundaries and face/accent layer order remains authoritative.
...
**Authorized moddability architecture direction:** new systemic features are designed around stable provider-qualified semantic IDs, engine-free definition registries, bounded property/capability descriptors, semantic events/actions and declarative behavior graphs so later safe content packs can create substantial new content without changing game C#. Workshop remains non-executable; package updates are hostile data and revalidated by content hash. Creator tooling, safe local content-pack compilation and broader Workshop declarative packs are planned phases, not claims that all of them already ship.
...
**Still deferred/forbidden without a new owner decision:** executable Workshop scripts/DLLs/native code; arbitrary custom Workshop Godot Resources/scenes/PCKs/shaders; generalized full-trust mod APIs; any claim that `AssemblyLoadContext` safely sandboxes untrusted C#; a Workshop WebAssembly runtime before a dedicated owner/security gate; Buddy-to-Buddy social AI/relationships/coordination/collision design; multiple independent player economy save slots; unapproved cosmetic progression/economy changes; unapproved painting tools such as material sliders/patterns/gradients/smudge/stamps/layers/blend modes/brush files/tablet pressure/3D orbit/back-side painting; Steam lobbies; matchmaking; P2P/network sockets/SDR; RPCs; `MultiplayerPeer`; shared live rooms; the 30-second friends Damage Sprint leaderboard; Linux release support; and macOS release support.
