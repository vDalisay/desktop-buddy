...
echo [2/10] Running domain tests for paint modifiers, fill, environment paint, and presentation response...
call dotnet test tests\DesktopBuddy.Domain.Tests\DesktopBuddy.Domain.Tests.csproj -c Debug --no-build --no-restore
if errorlevel 1 goto :failed

echo [3/10] Importing Godot project and validating autoload/script composition...
...
echo [4/10] Checking Paint Buddy trusted UV mapping...
call :scenario paint_frontal_uv_mapping
if errorlevel 1 goto :failed

echo [5/10] Checking semantic paint toolbar composition...
call :scenario paint_toolbar_icons
if errorlevel 1 goto :failed

echo [6/10] Checking Buddy Studio user-testing catalogue, preview, ownership, and save UX...
call :scenario buddy_studio_ui_composition
if errorlevel 1 goto :failed

echo [7/10] Checking unified Catalogue buy/equip behavior...
call :scenario shop_panel_purchase
if errorlevel 1 goto :failed

echo [8/10] Checking room decorator user-testing closure behavior...
call :scenario environment_decorator_room_build
if errorlevel 1 goto :failed

echo [9/10] Checking Work Mode resize/resilience/exit behavior...
call :scenario work_mode_resilience
if errorlevel 1 goto :failed

echo [10/10] Checking live 3D presentation remains coherent after rotation polish...
