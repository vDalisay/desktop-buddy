...
echo [4/9] Checking trusted cosmetic anchors, render layers, paint preservation, and physics isolation...
call :scenario character_rig_view
if errorlevel 1 goto :failed

echo [5/9] Checking permanent ownership, unowned preview, Save gating, Cancel, and restart restoration...
call :scenario buddy_studio_ownership_preview
if errorlevel 1 goto :failed

echo [6/9] Checking twelve-category deterministic owned/free Randomize...
call :scenario buddy_studio_randomize
if errorlevel 1 goto :failed

echo [7/9] Checking Studio composition, catalogue, purchase/equip, transforms, focus, and dirty-close behavior...
call :scenario buddy_studio_ui_composition
if errorlevel 1 goto :failed

echo [8/9] Running character save/use/restart plus Paint Buddy preservation journey...
call :scenario character_editor_create_use_and_react
if errorlevel 1 goto :failed

echo [9/9] Checking normal-boot Customize ^> Buddy Studio registration and opening...
