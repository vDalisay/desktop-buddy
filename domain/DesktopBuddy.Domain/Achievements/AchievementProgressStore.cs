...
/// <summary>
/// Typed access to versioned achievement state stored in account-global Player progress.
/// Qualification is monotonic account state; counters and working values are resettable progress.
/// The store deliberately knows nothing about Godot, Steam, Scenes or a particular Buddy actor.
/// </summary>
public sealed class AchievementProgressStore
{
    ...
    public AchievementProgressStore(PlayerProgressState player)
    {
    ...
    public bool Qualify(string achievementId)
    {
        ...
        _player.SetExtensionValue(QualifiedPrefix + achievementId, "1");
    ...
    public void SetValue(string name, string value)
    {
        ...
        _player.SetExtensionValue(ValuePrefix + name, value ?? string.Empty);
    }

    /// <summary>
    /// Reset Progress retains only already-qualified awards. Steam awards cannot be revoked and
    /// Next-Fest-qualified awards must still reconcile in Full Release; counters and transient rule
    /// values intentionally disappear with ordinary resettable progress.
    /// </summary>
    ...
    {
        if (_player.Extensions?.Values is { } values && values.TryGetValue(key, out string? found))
        {
            value = found;
            return true;
        }
