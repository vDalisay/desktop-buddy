...
echo [4/41] Checking laboratory controls...
...
echo [5/41] Checking grab and throw...
...
echo [6/41] Checking grab hang orientation...
...
echo [7/41] Checking grab swing pendulum...
...
echo [8/41] Checking room resize and zoom...
...
echo [9/41] Running lab spawn/settle journey...
...
echo [10/41] Running grab-throw journey...
...
echo [11/41] Checking dual-profile composition...
...
echo [12/41] Checking M4 object catch/hold...
...
echo [13/41] Checking M4 behavior priority ladder...
...
echo [14/41] Checking M4 hidden lifecycle accrual...
...
echo [15/41] Running M4 care/persistence relaunch journey...
...
echo [16/41] Checking clean-catch laugh and fun interest...
...
echo [17/41] Checking M5 Baseball purchase and pullback launch...
...
echo [18/41] Checking cornered ground pickup...
...
echo [19/41] Checking the FR-014 loose-object budget...
...
echo [20/41] Checking the M5 Meal consume and cooldown...
...
echo [21/41] Running the M5 Meal journey...
...
echo [22/41] Checking the M5 Baseball Bat swing...
...
echo [23/41] Running the M5 Baseball Bat journey...
...
echo [24/41] Running the M5 Home-Run Bat journey...
...
echo [25/41] Checking the M5 Pistol cadence, reload, and shot...
...
echo [26/41] Running the M5 Pistol journey...
...
echo [27/41] Checking the M5 Grenade pin, fuse, and blast...
...
echo [28/41] Running the M5 Grenade journey...
...
echo [29/41] Checking the M5 Fire Sprayer stream and Burning...
...
echo [30/41] Running the M5 Fire Sprayer journey...
...
echo [31/41] Checking the M5 Soccer Ball bounce and the Drink's care rules...
...
echo [32/41] Running the M5 Soccer Ball journey...
...
echo [33/41] Running the M5 Drink journey...
...
echo [34/41] Checking the M5 Shotgun spread, dedup, and shell eject...
...
echo [35/41] Running the M5 Shotgun journey...
...
echo [36/41] Checking the M5 Repair Kit application and clears...
"%GODOT_HEADLESS%" --headless --fixed-fps 120 --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\repair_kit.log" -- --scenario=repair_kit --seed=1 --artifacts=.artifacts\quick\repair_kit
if errorlevel 1 goto :failed

echo [37/41] Running the M5 Repair Kit journey...
"%GODOT_HEADLESS%" --headless --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\m5_repair_kit.log" -- --journey=m5_repair_kit --seed=1 --artifacts=.artifacts\quick\m5_repair_kit
if errorlevel 1 goto :failed

echo [38/41] Checking Power Grab against Normal Grab...
"%GODOT_HEADLESS%" --headless --fixed-fps 120 --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\power_grab.log" -- --scenario=power_grab --seed=1 --artifacts=.artifacts\quick\power_grab
if errorlevel 1 goto :failed

echo [39/41] Running the M5 Power Grab journey...
"%GODOT_HEADLESS%" --headless --fixed-fps 120 --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\m5_power_grab.log" -- --journey=m5_power_grab --seed=1 --artifacts=.artifacts\quick\m5_power_grab
if errorlevel 1 goto :failed

echo [40/41] Running the M5 economy calibration benchmark...
"%GODOT_HEADLESS%" --headless --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\economy_calibration.log" -- --scenario=economy_calibration --seed=1 --artifacts=.artifacts\quick\economy_calibration
if errorlevel 1 goto :failed

echo [41/41] Running the M5 shop progression journey...
"%GODOT_HEADLESS%" --headless --fixed-fps 120 --path . --rendering-driver opengl3 --log-file "%PROJECT_ROOT%\.artifacts\quick\m5_shop_progression.log" -- --journey=m5_shop_progression --seed=1 --artifacts=.artifacts\quick\m5_shop_progression
if errorlevel 1 goto :failed
