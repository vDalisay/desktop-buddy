...
/// <summary>
/// glasses@2 semantic generator. Unlike glasses@1, source pixels are already authoring-space:
/// the 1024x1024 Buddy-head template is the placement contract, so this generator never recenters
/// or rescales the artist's drawing. Lens openings drive the frame loops, an authored bridge is
/// traced from the painted pixels between those lenses, and temples remain trusted/generated.
/// </summary>
public static class GlassesTemplateV2Generator
{
    private const float SimplificationToleranceCells = 1.25f;
    private const float BridgeSimplificationToleranceCells = 1.0f;
        ...
        out CanonicalMesh mesh)
    {
        ...
        // The bridge/nose is authored. We do not synthesize a replacement straight bridge in v2.
        // A user can draw it straight, curved, raised, lowered, or omit it entirely.
        IReadOnlyList<Vector2> bridgeGrid = BuildAuthoredBridgePath(grid, left, right);
        if (bridgeGrid.Count >= 2)
        {
            List<Vector2> bridgeWorld = ToHeadLoop(grid, bridgeGrid);
            List<Vector2> bridgeUv = ToPaintUvLoop(grid, foreground, bridgeGrid);
            AddOpenPlanarTubePath(
                mesh,
                bridgeWorld,
                bridgeUv,
                frameRadius,
    ...
    private static IReadOnlyList<Vector2> BuildAuthoredBridgePath(
        MaskGrid grid,
        HoleContour left,
        HoleContour right)
    {
        float leftInnerX = left.GridPoints.Max(static p => p.X);
        float rightInnerX = right.GridPoints.Min(static p => p.X);
        int x0 = Math.Clamp((int)MathF.Ceiling(leftInnerX) + 1, 0, grid.Width - 1);
        int x1 = Math.Clamp((int)MathF.Floor(rightInnerX) - 1, 0, grid.Width - 1);
        if (x1 <= x0) return Array.Empty<Vector2>();

        float leftHeight = left.GridPoints.Max(static p => p.Y) - left.GridPoints.Min(static p => p.Y);
        float rightHeight = right.GridPoints.Max(static p => p.Y) - right.GridPoints.Min(static p => p.Y);
        float centerY = (left.Centroid.Y + right.Centroid.Y) * 0.5f;
        int halfCorridor = Math.Max(10, (int)MathF.Ceiling(MathF.Max(leftHeight, rightHeight) * 0.70f));
        int y0 = Math.Clamp((int)MathF.Floor(centerY) - halfCorridor, 0, grid.Height - 1);
        int y1 = Math.Clamp((int)MathF.Ceiling(centerY) + halfCorridor, 0, grid.Height - 1);
        int stride = Math.Max(1, grid.Width / 256);
        ...
        for (int x = x0; x <= x1; x += stride)
        {
            var ys = new List<int>();
            for (int y = y0; y <= y1; y++)
                if (grid[x, y]) ys.Add(y);
            if (ys.Count == 0) continue;

            // Median follows the centre of an authored brush stroke and is stable for even-width
            // strokes. It also lets arches/slopes survive without treating stroke width as 3D size.
            float median = ys.Count % 2 == 1
                ? ys[ys.Count / 2]
                : (ys[ys.Count / 2 - 1] + ys[ys.Count / 2]) * 0.5f;
            samples.Add(new Vector2(x, median));
        }

        if (samples.Count < 2) return Array.Empty<Vector2>();

        int leftAttach = IndexClosestTo(left.GridPoints, samples[0]);
        int rightAttach = IndexClosestTo(right.GridPoints, samples[^1]);
        var path = new List<Vector2>(samples.Count + 2)
        {
            left.GridPoints[leftAttach],
        };
        path.AddRange(samples);
        path.Add(right.GridPoints[rightAttach]);
        return SimplifyOpen(path, BridgeSimplificationToleranceCells);
    }

    private static int IndexClosestTo(IReadOnlyList<Vector2> points, Vector2 target)
    {
        int best = 0;
        float distance = Vector2.DistanceSquared(points[0], target);
        for (int i = 1; i < points.Count; i++)
        {
            float candidate = Vector2.DistanceSquared(points[i], target);
            if (candidate < distance - 1e-8f)
            {
                best = i;
                distance = candidate;
            }
    ...
    private static List<Vector2> SimplifyOpen(IReadOnlyList<Vector2> source, float tolerance)
    {
        ...
        var points = source.ToList();
        for (int pass = 0; pass < 8 && points.Count > 2; pass++)
        {
            bool changed = false;
            var reduced = new List<Vector2> { points[0] };
            for (int i = 1; i < points.Count - 1; i++)
            {
                if (DistanceToSegment(points[i], reduced[^1], points[i + 1]) <= tolerance)
                {
                    changed = true;
                    continue;
                }
    ...
    private static List<Vector2> ToHeadLoop(MaskGrid grid, IReadOnlyList<Vector2> points)
    {
        ...
        foreach (Vector2 point in points)
            result.Add(GlassesTemplateSpace.GridPointToHead(grid, point));
        return result;
    }
    ...
    private static void AddOpenPlanarTubePath(
        CanonicalMesh mesh,
        IReadOnlyList<Vector2> centers,
        IReadOnlyList<Vector2> uvs,
        float radiusXY,
        ...
        int radialSegments)
    {
        if (centers.Count < 2 || centers.Count != uvs.Count) return;
        uint[,] rings = new uint[centers.Count, radialSegments];
        for (int i = 0; i < centers.Count; i++)
        {
            Vector2 tangent = i switch
            {
                0 => centers[1] - centers[0],
                _ when i == centers.Count - 1 => centers[^1] - centers[^2],
                _ => centers[i + 1] - centers[i - 1],
            };
            tangent = tangent.LengthSquared() <= 1e-10f ? Vector2.UnitX : Vector2.Normalize(tangent);
            Vector2 normal = new(-tangent.Y, tangent.X);
            for (int r = 0; r < radialSegments; r++)
        ...
        {
            int s = (r + 1) % radialSegments;
            mesh.AddTriangle(rings[i, r], rings[i + 1, s], rings[i + 1, r]);
            mesh.AddTriangle(rings[i, r], rings[i, s], rings[i + 1, s]);
        }
    }
