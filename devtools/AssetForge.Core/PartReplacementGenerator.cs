...
public static class PartReplacementGenerator
{
    ...
    public static CanonicalMesh Generate(MaskGrid grid, GeometrySettings settings, AssetCategory category)
    {
        ...
        uint Vertex(int vx, int vy, bool front)
        {
            ...
            uint created = mesh.AddVertex(new Vector3(mapped.X, mapped.Y, front ? surfaceHalf : -surfaceHalf), new Vector2((float)vx / grid.Width, (float)vy / grid.Height));
    ...
    private static float SurfaceHalfDepth(MaskGrid grid, int[] inwardDistance, int maximumInset, int vx, int vy, float halfDepth, GeometrySettings settings)
    {
