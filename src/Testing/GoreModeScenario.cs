...
public sealed class GoreModeScenario : IScenario
{
    ...
    public async Task<ScenarioResult> RunAsync(SceneTree tree, ulong seed)
    {
        ...
        bool? restoreItch = DemoScope.ItchIoOverride;
        try
        {
        ...
        {
            DemoScope.ItchIoOverride = restoreItch;
            instance.QueueFree();
        }
    ...
    private static async Task Run(SceneTree tree, SandboxRoot sandbox, List<StartupCheck> checks)
    {
        ...
        // --- the build gate: the itch.io build refuses gore outright ---
        DemoScope.ItchIoOverride = true;
        sandbox.ApplyEffectsSettings(EffectsSettings.Default with { Gore = true });
        int before = gore.WoundsOpened;
        Wound(sandbox);
        await Ticks(tree, 2);
        checks.Add(new StartupCheck(
            "the_itch_build_refuses_gore_even_when_the_setting_says_yes",
        ...
        DemoScope.ItchIoOverride = false;
