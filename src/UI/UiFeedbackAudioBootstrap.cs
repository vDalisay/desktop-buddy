...
public partial class UiFeedbackAudioBootstrap : Node
{
    ...
    public override void _Ready()
    {
        ...
        _player = new AudioStreamPlayer
        {
            ...
            ProcessMode = ProcessModeEnum.Always,
            VolumeDb = -14.0f,
            // Fast clicking overlaps instead of cutting the previous press off mid-clip.
            MaxPolyphony = 8,
        };
        AddChild(_player);
    ...
    public void Play(UiFeedbackCue cue)
    {
        ...
        // Assigning Stream stops whatever is sounding, which would defeat the polyphony, so
        // a repeat of the same cue only re-triggers.
    ...
    private void PlayLayer(AudioStream? stream)
    {
        if (stream is null || !GodotObject.IsInstanceValid(_layerPlayer))
            return;
        if (_layerPlayer.Stream != stream)
            _layerPlayer.Stream = stream;
        _layerPlayer.Play();
    }
