    ...
    internal bool GeneratedReplacementPaintUvSeamIsCorrectForTest
    {
        get
        {
            ...
            void Check(Node3D? root, BuddyPartId part)
            {
                ...
                if (surface?.Mesh is not ArrayMesh mesh)
                {
                    ok = false;
                    return;
                }
                ...
                any = true;
                ok &= PaintMeshHasSplitFrontBackUv(
                    mesh,
                    Mesh.ArrayType.TexUV2,
                    part is BuddyPartId.LeftFoot or BuddyPartId.RightFoot ? 0.5f : 1f);
            }
        }
    }

    private static bool PaintMeshHasSplitFrontBackUv(ArrayMesh mesh, Mesh.ArrayType uvType, float uvScale)
    {
        bool hasFront = false;
        bool hasBack = false;
        ...
        for (int surfaceIndex = 0; surfaceIndex < mesh.GetSurfaceCount(); surfaceIndex++)
        {
            ...
            Vector2[] uvs = arrays[(int)uvType].AsVector2Array();
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                foreach (float existing in values)
                    if (Mathf.Abs(existing - uv.X) > (0.49f * uvScale))
            ...
            bool CheckTriangle(int ia, int ib, int ic)
            {
                ...
                {
                    hasBack = true;
                    return minU >= (0.4999f * uvScale);
                }

                hasFront = true;
                return maxU <= (0.5001f * uvScale);
            }
        }
